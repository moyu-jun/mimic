...
fn copy_and_verify_resources() {
    ...
    let source = Path::new(&manifest_dir)
        ...
        .join("extra");
    let destination = Path::new(&manifest_dir).join("target").join(profile);
    ...
    for relative in REQUIRED_RESOURCES {
        ...
        if !copied.is_file() {
            panic!("required copied resource {relative} is missing");
        }
    }
}

fn verify_installer_hash(path: &Path) {
    let mut file = fs::File::open(path).expect("failed to open driver installer for hashing");
    let mut hasher = Sha256::new();
    let mut buffer = [0_u8; 64 * 1024];
    loop {
        let read = file
            .read(&mut buffer)
            .expect("failed to hash driver installer");
        if read == 0 {
            break;
        }
